...
metadata = {
    'protocolName': 'Isotherm Step 2 - Equilibration',
    'author': 'S. Putz',
    'description': "Equilibration of Filterplate"
}
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    # =====================
    REPLICATES_PLACEHOLDER = "3"
    EQ_VOL_PLACEHOLDER = "0"    # in uL per well
    SHAKER_SPEED_PLACEHOLDER = "900"  # RPM
    CYCLE_DURATION_PLACEHOLDER = "0"  # minutes
    ...
    equilibration_volume_ul = parse_float(EQ_VOL_PLACEHOLDER, 100)
    ...
    tiprack_slot7.set_offset(-0.4,1.5,0.0)
    # Reservoirs and mixing plate (as specified deck layout)
    reservoir_3 = protocol.load_labware('nest_12_reservoir_15ml', 6)   # Reservoir 3 (buffers)
    reservoir_2 = protocol.load_labware('nest_12_reservoir_15ml', 8)   # Reservoir 2
    reservoir_1 = protocol.load_labware('nest_12_reservoir_15ml', 9)   # Reservoir 1 (equilibration)
    ...
    # Use multi-channel, tips from slot 4, always return tips
    # =====================
    # Prepare source wells and destination columns
    source_wells = reservoir_1.wells()[:max_positions]  # A1..A12 in order (Well[0] -> A1, etc.)
    ...
    if max_positions > 0 and equilibration_volume_ul > 0:
        p300m.pick_up_tip()
        p300m.transfer(
